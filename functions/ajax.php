...
function OT_lightbox_gallery(){
	...
	foreach($imageIDs as $id) {
		...
		$image = get_post_thumb(false, 1200, 0, false, $file);
		$images[] = $image['src'];
		$thumb = get_post_thumb(false, 80, 80, false, $file);
		$thumbs[$c] = $thumb['src'];
		$c++;
	}
...
function footer_contact_form() {
	...
	if(isset($_POST["form_type"])) {	
		...
		$subject = ( __('From','quadrum-theme')
)." ".get_bloginfo('name')." ".( __('Contact Page','quadrum-theme')
);
		...
		ob_start(); 
		?>
<?php printf ( __('Message:','quadrum-theme')
);?> <?php echo nl2br($message);?>
<div style="padding-top:100px;">
<?php printf ( __('Name:','quadrum-theme')
);?> <?php echo $u_name;?><br/>
<?php printf ( __('E-mail:','quadrum-theme')
);?> <?php echo $email;?><br/>
<?php printf ( __('IP Address:','quadrum-theme')
);?> <?php echo $ip;?><br/>
</div>
<?php
...
function quadrum_visitor_vote() {
	if ( isset($_REQUEST) && isset($_REQUEST['vote']) && isset($_REQUEST['nonce']) && isset($_REQUEST['post_id']) ) {
		check_ajax_referer( 'ot-action-quadrum' , 'nonce' );
		$id = $_REQUEST['post_id'];
		$vote = $_REQUEST['vote'];
		...
		if($ratings_count>0) {
			$visitor_rating = ($_ratings+$vote)/($ratings_count+1);
		} else {
			$visitor_rating = $vote;
		}
		

		update_post_meta( $id, THEME_NAME."_visitor_ratings_c", $ratings_count+1 );
		update_post_meta( $id, THEME_NAME."_visitor_ratings", $_ratings+$vote );


		update_post_meta( $id, THEME_NAME."_visitor_rating", $visitor_rating );


		//set the visitors and editors average rate
		$i=0;
		...
		if(!empty($totalRate)) { 
			...
			$ratings_average_2 = round((($total/$rateCount)/20),2);
		}


		update_post_meta( $id, THEME_NAME."_ratings_average_2", $ratings_average_2 );

		setcookie("quadrum_vote_".$id, '1', strtotime( '+300 days' ), COOKIEPATH, COOKIE_DOMAIN );

		echo $ratings_average_2;
...
add_action('wp_ajax_quadrum_visitor_vote', 'quadrum_visitor_vote');
add_action('wp_ajax_nopriv_quadrum_visitor_vote', 'quadrum_visitor_vote');
