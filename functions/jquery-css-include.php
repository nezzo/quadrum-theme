...
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly
	...
	function orange_themes_scripts() { 
		...
		wp_enqueue_style("main-stylesheet", THEME_CSS_URL."main-stylesheet.min.css", Array());
		wp_enqueue_style("lightbox", THEME_CSS_URL."lightbox.css", Array());
		wp_enqueue_style("shortcodes", THEME_CSS_URL."shortcodes.css", Array());
		wp_enqueue_style("ot-menu", THEME_CSS_URL."ot-menu.css", Array());
		...
		wp_enqueue_script(THEME_JS_URL."-scripts" , THEME_JS_URL."theme-scripts.min.js", Array('jquery'), "1.5.19", true);
		

		wp_enqueue_script("lightbox" , THEME_JS_URL."lightbox.min.js", Array('jquery'), "1.0", true);
		...
		wp_enqueue_script("ot-menu" , THEME_JS_URL."ot-menu.js", Array('jquery'), '', true);
		wp_enqueue_script("scripts-wp" , THEME_JS_URL.THEME_NAME.".js", Array('jquery'), "1.0.0", true);
		
		if ( is_singular() ) wp_enqueue_script( "comment-reply" );
		
		wp_enqueue_script("ot-gallery" , THEME_JS_URL."ot_gallery.min.js", Array('jquery'), "1.0", true);
		wp_enqueue_script("ot-scripts" , THEME_JS_URL."scripts.min.js", Array('jquery'), "1.0", true);
		...
		wp_localize_script('jquery','ot',
			array(
				...
				'themeUrl' => THEME_URL,
				'security' => wp_create_nonce( 'ot-action-quadrum'),
			)
		);
